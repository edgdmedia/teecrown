// About page.
const AB = window.TeeCrownConsultDesignSystem_08f0d5;
...
function VisionMission() {
  ...
  return (
    <div className="tcc-grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
      {cards.map((c, n) => (
...
function AboutPage() {
  const reasons = window.TCC.reasons;
  return (
    <PageShell current="About">
      {({ openContact }) => (
        <>
          <PageHero eyebrow="About Tee'Crown" title="Travel with people who genuinely care" lede="A proudly Nigerian team on a mission to make global travel simple, responsible and unforgettable." image="assets/hero-travel-2.jpg" />
          ...
          <Section>
            <div className="tcc-split" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '56px', alignItems: 'center' }}>
              <Reveal style={{ borderRadius: '14px', overflow: 'hidden', boxShadow: 'var(--shadow-card-hover)' }}>
                <img src="assets/tour-kenya.jpg" alt="Our travellers" style={{ width: '100%', height: '460px', objectFit: 'cover', display: 'block' }} />
              </Reveal>
              <div>
          ...
          <Section tint="alt">
            ...
            <div style={{ marginTop: 'var(--space-lg)' }}><VisionMission /></div>
          </Section>
          ...
          <section style={{ position: 'relative', overflow: 'hidden', color: '#fff' }}>
            <div style={{ position: 'absolute', inset: 0, background: "url('assets/blog-makoko.jpg') center/cover no-repeat" }} />
